...
// Display title for a show. Uses the show's own title when present; otherwise
// falls back to "<DJ name>'s show". Only when there's no DJ name to build that
// from do we land on the generic "Untitled show". Pass an explicit `djName` to
// override the show's own fields (e.g. on a DJ's page, where the name comes
// from the DJ record rather than the show row).
export function showTitleOrDefault(show = {}, djName) {
    const title = (show.title || "").trim();
    if (title) return title;
    const name = (djName || show.defdjname || show.djname || "").trim();
    return name ? `${name}'s show` : "Untitled show";
}
