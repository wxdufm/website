...
function parseSchedule() {
	...
	const scheduleCarrier = [
		...
		[], //idGrid
		[] //displayOverrideGrid: custom display names for [X]/[SP] cells, so their
		   //shown text survives even when a DJ id is pinned for the click-through
	]

	/*
	Cell conventions (see id-lookup.js):
	  "Last, First"                 -> looked up to a DJ id by name
	  "[SP] Show w/ Host"           -> specialty show (highlighted), custom display, no id
	  "[X] Custom Name"             -> custom display, skip name lookup, no id
	  "<anything> | <id>"           -> pins that DJ id for the click-through, keeping
	                                   the left-hand text as the display. Works with
	                                   any of the above (e.g. "[SP] Show w/ Host | 730").
	  "<anything> | <id>,<id>,..."  -> a show with multiple DJs; the click-through
	                                   links to all of them (e.g. "[X] Duo | 665,223").
	                                   The /fix-schedule-grid tool writes these pins.
	*/
